package totem.Guayando.managers;
...
public class LanguageManager {
    ...
    public LanguageManager(Toteminator plugin) {
        this.plugin = plugin;
        loadLanguageFile();
    }
    ...
    public void reloadConfig() {
        ...
        if (!selectedLangFile.exists()) {
            plugin.getLogger().warning(MessageUtils.getColoredMessage(Toteminator.prefix + " &4Language " + selectedLanguage + " does not exist in lang folder."));
            plugin.getLogger().warning(MessageUtils.getColoredMessage(Toteminator.prefix + " &4Using en.yml by default."));
    ...
    public String getMessage(String path) {
        return languageConfig.getString(path, MessageUtils.getColoredMessage(Toteminator.prefix + " &4Message &c" + path + " &4not found."));
    }

    public List<String> getStringList(String path) {
        List<String> messages = languageConfig.getStringList(path);
        if (messages.isEmpty()) {
            return Arrays.asList(MessageUtils.getColoredMessage(Toteminator.prefix + " &4Message &c" + path + " &4not found."));
        }
        return messages;
    }
}